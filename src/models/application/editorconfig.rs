...
impl EditorConfig {
    pub fn from_directory(path: &Path) -> Result<EditorConfig> {
        let mut pathbuf = PathBuf::from(path);
        pathbuf.push(".editorconfig");

        load_from_file(&pathbuf)
            .map(|sections| EditorConfig { sections })
...
fn load_from_file(path: &Path) -> Result<Vec<Section>> {
    let file = File::open(path)
        .chain_err(|| "Failed to load .editorconfig file")?;
