...
// FIXME: ThemeMode shouldn't be mutable, but the trait needs a
//        mutable reference to the query for its push/pop methods.
//        This will go away if/when trait field support is added.
pub fn display(workspace: &mut Workspace, mode: &mut ThemeMode, view: &mut View) {
    ...
    if mode.results().count() == 0 {
        view.print(&Position{ line: 0, offset: 0 },
                   Style::Default,
                   Colors::Default,
                   &"No matching themes found.".pad_to_width(view.width()));
     }
